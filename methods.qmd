...
read_gbe_files <- function(file, variables) {
  ...
  file_with_folder = paste0("data/", file)
  
  # Preread thte data to find parameters vor read_csv2
  testread = readLines(file_with_folder, encoding = "UTF-8")
  ...
  data <- suppressMessages(
          suppressWarnings(
              read_csv2(file = file_with_folder, 
                    skip = beginning,
                    n_max = end,
    ...
    # Fill the variables 
    fill(starts_with("..."), .direction = "down") 
    
  # adjust names with the argument of this function (variable) to replace the "..1" variable names
    names(data) <- c(variables, names(data)[(1+length(variables)):length(names(data))])
...
## Abbildung
```{r, Abbildung 1}
data <- read_gbe_files(file = "Taetigkeit_Facharzt.csv", variables = c("Facharzt", "Taetigkeit"))
...
ggsave(p1, filename = paste0("graphs/", Sys.Date(), "_Abbildung_1"), device = "jpg", width = 28, height = 14, units = "cm", dpi = 300)
